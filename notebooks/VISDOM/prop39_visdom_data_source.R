...
MyDataSource = function(dbConfig='data_db.cfg', queryCache=paste(getwd(),'/','DATA_CACHE','/',sep='')){
  ...
  obj$getAllData = function(geocode,useCache=F,forceRefresh=F) {
    ...
    # convert last 96 columns to kWh from Wh
    data[,c(-95:0) + ncol(data)] = data[,c(-95:0) + ncol(data)] / 1000
    return(data)
  }
  ...
  obj$getMeterData = function(id,geocode=NULL) {
    ...
    # convert last 96 columns to kW from W
    data[,c(-95:0) + ncol(data)] = data[,c(-95:0) + ncol(data)] / 1000
  ...
  obj$getGeoForId = function(id,useCache=F, forceRefresh=F) {
    query    <- paste("select meter_uuid as id, zip5 from",obj$accountTable,' group by id order by id')
    cacheFile = NULL
